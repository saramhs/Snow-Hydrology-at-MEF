...
```{r contribution bar plot}
contribution_season<-cont_opt_stf_df%>%mutate(season=stream_projection_season$season)%>%mutate(DoY=stream_projection_season$DoY)
contribution_long <- contribution_season %>% pivot_longer(cols = c(F1, F2, F3), names_to = "Variable", values_to ="Value") 
variable_labels <- c("F1" = "SSF", "F2" = "Bog", "F3" = "Lagg") 
custom_colors <- c("F1" = "chocolate4","F2"="#FED300","F3"="#6b7701")
...
cont_box<-ggplot(contribution_long, aes(x = Variable, y = Value, fill = Variable)) + geom_boxplot() +
  ...
  labs(x = "Variable", y = "Value", fill ="Season") +
  theme_classic()+
